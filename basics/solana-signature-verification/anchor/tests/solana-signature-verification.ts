...
import { Connection, Ed25519Program } from '@solana/web3.js';
...
describe('solana-signature-verification', () => {
  ...
  before(async () => {
    ...
    priceUpdateAccount = pythSolanaReceiver.getPriceFeedAccountAddress(shardId, SOL_PRICE_FEED_ID).toBase58();
    ...
    try {
      ...
      tx.add(
        await program.methods
          .deposit(amountToLockUp, unlockPrice)
          .accounts({
      ...
      const blockhash = (await provider.connection.getLatestBlockhash('finalized')).blockhash;
  ...
  it('withdraws from escrow', async () => {
    ...
    const userBalanceBefore = await provider.connection.getBalance(temp_payer.publicKey);
    ...
    try {
      ...
      assert(userBalanceAfter > userBalanceBefore, 'User balance should have increased');
