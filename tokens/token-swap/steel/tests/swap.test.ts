...
describe("Testing Swap A for B", async () => {
  ...
  it("should swap token A for B successfully", async () => {
    ...
    let tx = createSwapInstruction(values, payer, context);
    await client.processTransaction(tx);

    const traderTokenAccountA = TokenLayout.decode(
      (await client.getAccount(values.holderAccountA)).data
    );
    const traderTokenAccountB = TokenLayout.decode(
      (await client.getAccount(values.holderAccountB)).data
    );

    expect(traderTokenAccountA.amount.toString()).to.equal(
      values.defaultSupply.sub(values.depositAmountA).sub(input).toString()
    );
    expect(Number(traderTokenAccountB.amount.toString())).to.be.greaterThan(
      values.defaultSupply.sub(values.depositAmountB).toNumber()
    );
    expect(Number(traderTokenAccountB.amount.toString())).to.be.lessThan(
  ...
  it("should fail to swap if the output is smaller than the min_output_amount", async () => {
    ...
    let tx = createSwapInstruction(values, payer, context, input, output);
  ...
  it("should not alter balances if swap amount is zero", async () => {
    ...
    let tx = createSwapInstruction(values, payer, context, input, input);
    await client.processTransaction(tx);
    ...
    expect(traderTokenAccountB.amount.toString()).to.equal(
