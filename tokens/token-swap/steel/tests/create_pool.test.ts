...
import { createAmmTransactionInstruction, createPoolInstruction } from './transactions';
...
describe('Testing Creaion of Pool', async () => {
  ...
  it('creates a pool successfully', async () => {
    ...
    let poolAccount: Pool | AccountInfoBytes = await client.getAccount(values.poolKey);
    poolAccount = PoolLayout.decode(poolAccount.data);

    expect(poolAccount.amm.toString()).to.equal(values.ammKey.toString());
    expect(poolAccount.mintA.toString()).to.equal(values.mintAKeypair.publicKey.toString());
    expect(poolAccount.mintB.toString()).to.equal(values.mintBKeypair.publicKey.toString());
  });
