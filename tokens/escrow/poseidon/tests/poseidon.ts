...
import { ComputeBudgetProgram, LAMPORTS_PER_SOL, PublicKey, SystemProgram, Transaction, type TransactionInstruction } from '@solana/web3.js';
...
describe('Escrow program', async () => {
    ...
    await provider.connection.sendTransaction(
      new Transaction().add(
        ComputeBudgetProgram.setComputeUnitLimit({
          units: 1400000  // Increased compute units
        }),
        ComputeBudgetProgram.setComputeUnitPrice({
          microLamports: 1  // Adjust price as needed
        })
      ),
      [tokenMintA]
    )
  ...
  const make = async () => {
    ...
    const transactionSignature = await program.methods
      .make(offerId, tokenAOfferedAmount, tokenBWantedAmount)
      .accounts({ ...accounts })
      .signers([alice])
      .rpc();
    ...
    const offerAccount = await program.account.escrowState.fetch(offer);

    assert(offerAccount.maker.equals(alice.publicKey));
    assert(offerAccount.makerMint.equals(accounts.tokenMintA));
    assert(offerAccount.takerMint.equals(accounts.tokenMintB));
