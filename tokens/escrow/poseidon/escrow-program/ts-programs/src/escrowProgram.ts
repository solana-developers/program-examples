...
export default class EscrowProgram {
  static PROGRAM_ID = new Pubkey('BCsW6bJJZPgLYba52dNie4L2SebE7ALHgQyWakLehvSz');
  ...
  take(
    ...
    takerAta.derive(makerMint, taker.key).initIfNeeded();

    takerReceiveAta.derive(makerMint, taker.key).initIfNeeded();

    makerAta.derive(makerMint, maker.key);

    escrow.derive(['escrow', maker.key, escrow.seed.toBytes()]).has([maker, makerMint, takerMint]).close(maker);
