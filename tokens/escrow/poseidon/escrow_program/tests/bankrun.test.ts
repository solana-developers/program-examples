...
import type { Escrow } from '../target/types/escrow_program';
...
describe('Escrow Bankrun example', async () => {
  ...
  const make = async () => {
    // Pick a random ID for the escrow we'll make
    const escrowId = getRandomBigNumber();

    // Then determine the account addresses we'll use for the escrow and the vault
    const escrow = PublicKey.findProgramAddressSync(
      [Buffer.from('escrow'), accounts.maker.toBuffer(), escrowId.toArrayLike(Buffer, 'le', 8)],
      program.programId,
    )[0];

    const vault = getAssociatedTokenAddressSync(accounts.tokenMintA, escrow, true, TOKEN_PROGRAM);

    accounts.escrow = escrow;
    accounts.vault = vault;

    const transactionSignature = await program.methods
      .makeOffer(escrowId, tokenAOfferedAmount, tokenBWantedAmount)
      .accounts({ ...accounts })
      .signers([alice])
      .rpc();
    ...
    assert(escrowAccount.tokenMintA.equals(accounts.tokenMintA));
    assert(escrowAccount.tokenMintB.equals(accounts.tokenMintB));
    assert(escrowAccount.tokenBWantedAmount.eq(tokenBWantedAmount));
  };
