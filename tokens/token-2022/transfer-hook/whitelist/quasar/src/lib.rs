...
impl InitializeExtraAccountMetaList<'_> {
    ...
    pub fn initialize(&self) -> Result<(), ProgramError> {
        ...
        if self.white_list.to_account_view().address() != &wl_pda {
            return Err(ProgramError::InvalidSeeds);
        }
        ...
        self.system_program
            .create_account(self.payer, &*self.white_list, wl_lamports, wl_size, &crate::ID)
            .invoke_signed(&wl_seeds)?;

        // Write authority (payer) to whitelist account
        let wl_view = unsafe {
            &mut *(self.white_list as *const UncheckedAccount as *mut UncheckedAccount
                as *mut AccountView)
        };
        let mut wl_data = wl_view.try_borrow_mut()?;
        wl_data[0..32].copy_from_slice(self.payer.to_account_view().address().as_ref());
...
impl TransferHook<'_> {
    ...
    pub fn transfer_hook(&self) -> Result<(), ProgramError> {
        let wl_view = self.white_list.to_account_view();
        ...
        let dest_address = self.destination_token.to_account_view().address();
        let mut found = false;
        for i in 0..count {
            let offset = 36 + i * 32;
...
impl AddToWhitelist<'_> {
    #[inline(always)]
    pub fn add_to_whitelist(&self) -> Result<(), ProgramError> {
        let view = unsafe {
            &mut *(self.white_list as *const UncheckedAccount as *mut UncheckedAccount
                as *mut AccountView)
        };
        ...
        let signer_address = self.signer.to_account_view().address();
        ...
        let new_address = self.new_account.to_account_view().address();
