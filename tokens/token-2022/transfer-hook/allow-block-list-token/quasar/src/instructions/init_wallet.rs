...
impl InitWallet<'_> {
    #[inline(always)]
    pub fn init_wallet(&self, allowed: bool) -> Result<(), ProgramError> {
        // Verify config PDA
        ...
        let config_view = self.config.to_account_view();
        ...
        if stored_authority != self.authority.to_account_view().address().as_ref() {
            return Err(errors::unauthorized());
        }
        drop(config_data);

        // Create ABWallet PDA
        let wallet_key = self.wallet.to_account_view().address();
        let (ab_wallet_pda, bump) = Address::find_program_address(
            &[AB_WALLET_SEED, wallet_key.as_ref()],
            &crate::ID,
        );
        if self.ab_wallet.to_account_view().address() != &ab_wallet_pda {
            return Err(ProgramError::InvalidSeeds);
        }
        ...
        // Write wallet data
        let view = unsafe {
            &mut *(self.ab_wallet as *const UncheckedAccount as *mut UncheckedAccount
                as *mut AccountView)
        };
